/**
 * Tonex (Tone Object Notation EXtended) 1.0 - Core Library
 * 
 * A JavaScript library for parsing, validating, and manipulating
 * musical compositions in the Tonex specification.
 * 
...
class Tonex {
    static VERSION = "1.0";
    static FORMAT_IDENTIFIER = "tonex";
    ...
    static processNoteInput(note) {
        if (Array.isArray(note)) {
            return note.map(n => Tonex.processNoteInput(n));
        } else if (typeof note === 'number') {
            return Tonex.midiNoteToNoteName(note);
    ...
    /**
     * Validate a tonex composition object
     * @param {object} composition - tonex composition to validate
    ...
    static validate(composition) {
        ...
        } else if (composition.format !== Tonex.FORMAT_IDENTIFIER) {
            errors.push(`Invalid format: expected "${Tonex.FORMAT_IDENTIFIER}", got "${composition.format}"`);
        }
     ...
     * @param {object} sequence - tonex sequence object
    ...
    static expandNotesWithLoop(sequence, totalDuration) {
        ...
        if (sequence.loop && sequence.loop !== false) {
            ...
            if (typeof sequence.loop === 'string') {
                loopEndTime = Tonex._parseTimeString(sequence.loop);
            } else if (sequence.loop === true) {
                // Calculate from notes
                const lastNoteTime = Math.max(...sequence.notes.map(n => {
                    const noteTime = typeof n.time === 'string' ? Tonex._parseTimeString(n.time) : n.time;
                    const noteDuration = typeof n.duration === 'string' ? Tonex._parseTimeString(n.duration) : n.duration;
            ...
            for (let loopIndex = 1; loopIndex < numberOfLoops; loopIndex++) {
                ...
                sequence.notes.forEach(originalNote => {
                    const noteTime = typeof originalNote.time === 'string' ? 
                        Tonex._parseTimeString(originalNote.time) : originalNote.time;
     ...
     * @param {object} composition - tonex composition
    ...
    static calculateDuration(composition) {
        ...
        composition.sequences.forEach(seq => {
            ...
            if (seq.loop && seq.loop !== false && typeof seq.loop === 'string') {
                sequenceDuration = Tonex._parseTimeString(seq.loop);
            } else {
                // Calculate from notes
                seq.notes.forEach(note => {
                    const noteTime = typeof note.time === 'string' ? 
                        Tonex._parseTimeString(note.time) : note.time;
                    const noteDuration = typeof note.duration === 'string' ? 
                        Tonex._parseTimeString(note.duration) : note.duration;
        ...
        composition.sequences.forEach(seq => {
            if (seq.loop && seq.loop !== false && typeof seq.loop === 'string') {
                const loopTime = Tonex._parseTimeString(seq.loop);
    ...
    /**
     * Convert JSON tonex format to Tone.js compatible format
    ...
    static convertToToneFormat(jsonData) {
        console.log('🔄 Tonex: Converting enhanced JSON to Tone.js format...');
        
        // Validate basic structure
        if (!jsonData.sequences || !Array.isArray(jsonData.sequences)) {
            throw new Error('Invalid tonex format: missing sequences array');
        }
        ...
        console.log('✅ Tonex: Enhanced conversion complete');
    ...
    static convertSynthFormat(synthConfig) {
        ...
        console.log(`🎛️ Tonex: Converted ${synthConfig.type} synth with ${synthConfig.effects?.length || 0} effects`);
        return converted;
    }
    ...
    static convertGlobalEffects(globalEffects) {
        ...
        console.log('🌐 Tonex: Processing global effects:', Object.keys(globalEffects));
    ...
    static _parseTimeString(timeStr) {
        ...
        // This would need to be more sophisticated for full tonex support
